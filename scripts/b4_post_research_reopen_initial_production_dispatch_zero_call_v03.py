...
OLD = Path(".aic-runtime/b4_post_research_reopen_initial_production_dispatch_zero_call_preflight_v0_2.json")
OUT = Path(".aic-runtime/b4_post_research_reopen_initial_production_dispatch_zero_call_preflight_v0_3.json")
RESULT = Path(".aic-runtime/b4_post_research_reopen_initial_council_freeze_v0_1.json")
OLD_HASH = "68d623c089ab529bb1e0d8a6892f7c67be14d8c4239e6070526d5dbc2bf68578"
...
def main() -> int:
    try:
        ...
        if _git("branch", "--show-current") != EXPECTED_BRANCH or _git("status", "--porcelain"):
            raise PostResearchInitialExecutionError("v0.3 readiness requires clean target branch")
        if OUT.exists() or RESULT.exists():
            raise PostResearchInitialExecutionError("exclusive output or fresh result already exists")
        if _read_object(OLD, "immutable v0.2 readiness").get("artifact_hash") != OLD_HASH:
            raise PostResearchInitialExecutionError("immutable v0.2 readiness drift")
        ...
        print(f"NEW_READINESS_HASH={artifact['artifact_hash']}")
        ...
        print(f"B4_POST_RESEARCH_REOPEN_INITIAL_V03_READINESS_STOP: {exc}", file=sys.stderr)
        return 2
