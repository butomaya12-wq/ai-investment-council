...
from aic.research.reopen_judge_s00_scope_v02 import (
...
DEFAULT_OUTPUT = Path(".aic-runtime/b3_research_reopen_s00_scope_zero_call_v0_2.json")


def _args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build production-shaped Judge-triggered B3 research-reopen S00 scope V02 with zero provider/model calls."
    )
...
def _head_and_clean() -> str:
    ...
    if branch != "hackathon/alpaca-2026":
        raise RuntimeError("S00 scope V02 must run on hackathon/alpaca-2026")
    ...
    if status:
        raise RuntimeError("working tree must be clean before S00 scope V02")
    return head
...
def _summary(artifact: Mapping[str, Any], *, output: Path, write_state: str) -> dict[str, Any]:
    ...
    return {
        ...
        "supersedes_failed_v01_code_commit_sha": artifact["supersedes_failed_v01_code_commit_sha"],
    ...
    try:
        ...
        if args.output.exists():
            ...
            if dict(existing) != artifact:
                raise RuntimeError("existing S00 scope V02 artifact differs from deterministic rebuild")
        ...
        print(
            json.dumps(
                _summary(artifact, output=args.output, write_state=write_state),
                indent=2,
                ensure_ascii=False,
                sort_keys=True,
            )
