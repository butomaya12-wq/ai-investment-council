...
from aic.research.reopen_judge_s00_scope_v01 import (
...
DEFAULT_OUTPUT = Path(".aic-runtime/b3_research_reopen_s00_scope_zero_call_v0_1.json")


def _args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build the Judge-triggered B3 research-reopen S00 scope without provider/model calls."
    )
...
def _head_and_clean() -> str:
    ...
    if branch != "hackathon/alpaca-2026":
        raise RuntimeError("S00 scope must run on hackathon/alpaca-2026")
    ...
    if status:
        raise RuntimeError("working tree must be clean before S00 scope")
    return head
    ...
    try:
        ...
        if args.output.exists():
            ...
            if dict(existing) != artifact:
                raise RuntimeError("existing S00 scope artifact differs from deterministic rebuild")
