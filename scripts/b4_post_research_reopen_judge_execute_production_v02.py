"""Explicit paid v0.2 Judge entrypoint; inert without flag and approval."""
...
def main() -> int:
    ...
    parser.add_argument("--preflight", type=Path, default=ROOT / "b4_post_research_reopen_current_judge_preflight_zero_call_v0_2.json")
    parser.add_argument("--readiness", type=Path, default=ROOT / "b4_post_research_reopen_current_judge_readiness_zero_call_v0_2.json")
    ...
    parser.add_argument("--ledger", type=Path, default=ROOT / "b4_post_research_reopen_current_judge_paid_dispatch_ledger_v0_2.json")
    parser.add_argument("--raw", type=Path, default=ROOT / "b4_post_research_reopen_current_judge_raw_response_v0_2.json")
    parser.add_argument("--result", type=Path, default=ROOT / "b4_post_research_reopen_current_judge_council_freeze_v0_2.json")
    args = parser.parse_args()
    if not args.execute_paid_judge:
        raise SystemExit("STOP: --execute-paid-judge is required")
    closure, residual, gaps, initial, cost, rebuttal, preflight, readiness, selection, evaluation, pricing, approval, old = (_read(path) for path in (args.b3_closure, args.b3_residual_plan, args.b3_remaining_gaps, args.initial_freeze, args.initial_cost, args.rebuttal_freeze, args.preflight, args.readiness, args.selection, args.eval, args.pricing, args.owner_approval, args.historical_preflight))
    ...
    inputs = {"closure": closure, "residual_plan": residual, "remaining_gaps_closure": gaps, "initial_freeze": initial, "initial_cost": cost, "rebuttal_freeze": rebuttal, "entry": entry, "selection": selection, "eval_artifact": evaluation, "receipts": receipts, "pricing": pricing, "historical_request_hashes": [old["request_hash"]]}
    ...
    result = judge.execute_paid(execute_paid_judge=True, branch=_git("branch", "--show-current"), code_commit_sha=head, worktree_clean=not bool(_git("status", "--porcelain")), preflight=preflight, readiness=readiness, approval=approval, ledger_path=args.ledger, raw_path=args.raw, result_path=args.result, transport_factory=factory, **inputs)
