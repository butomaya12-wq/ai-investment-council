"""Materialize only current-lineage Judge preflight/readiness evidence."""
...
def main() -> int:
    ...
    parser.add_argument("--preflight-output", type=Path, default=ROOT / "b4_post_research_reopen_current_judge_preflight_zero_call_v0_1.json")
    parser.add_argument("--readiness-output", type=Path, default=ROOT / "b4_post_research_reopen_current_judge_readiness_zero_call_v0_1.json")
    ...
    closure, initial, cost, rebuttal, selection, evaluation, pricing, old = (_read(path) for path in (args.b3_closure, args.initial_freeze, args.initial_cost, args.rebuttal_freeze, args.selection, args.eval, args.pricing, args.historical_preflight))
    receipts = [json.loads(line) for line in args.receipts.read_text(encoding="utf-8").splitlines() if line]
    historical = [old["request_hash"]]
    head = _git("rev-parse", "HEAD")
    entry = judge.build_current_judge_entry(code_commit_sha=head, closure=closure, initial_freeze=initial, initial_cost=cost, rebuttal_freeze=rebuttal)
    preflight = judge.build_current_judge_preflight(code_commit_sha=head, closure=closure, initial_freeze=initial, initial_cost=cost, rebuttal_freeze=rebuttal, entry=entry, selection=selection, eval_artifact=evaluation, receipts=receipts, pricing=pricing, historical_request_hashes=historical)
    readiness = judge.build_current_judge_readiness(code_commit_sha=head, preflight=preflight, closure=closure, initial_freeze=initial, initial_cost=cost, rebuttal_freeze=rebuttal, entry=entry, selection=selection, eval_artifact=evaluation, receipts=receipts, pricing=pricing, historical_request_hashes=historical)
