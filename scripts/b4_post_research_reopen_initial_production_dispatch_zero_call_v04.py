...
OLD = Path(".aic-runtime/b4_post_research_reopen_initial_production_dispatch_zero_call_preflight_v0_3.json")
OUT = Path(".aic-runtime/b4_post_research_reopen_initial_production_dispatch_zero_call_preflight_v0_4.json")
RESULT = Path(".aic-runtime/b4_post_research_reopen_initial_council_freeze_v0_1.json")
OLD_HASH = "366d02ba6cb3de73b9e2a7376f72d519dfb7630a375937758c2409eec0c0813c"
...
def main() -> int:
    try:
        ...
        if _git("branch", "--show-current") != EXPECTED_BRANCH or _git("status", "--porcelain"):
            raise PostResearchInitialExecutionError("v0.4 readiness requires clean target branch")
        if OUT.exists() or RESULT.exists():
            raise PostResearchInitialExecutionError("exclusive output or fresh result already exists")
        if _read_object(OLD, "immutable v0.3 readiness").get("artifact_hash") != OLD_HASH:
            raise PostResearchInitialExecutionError("immutable v0.3 readiness drift")
        ...
        print(f"B4_POST_RESEARCH_REOPEN_INITIAL_V04_READINESS_STOP: {exc}", file=sys.stderr)
        return 2
