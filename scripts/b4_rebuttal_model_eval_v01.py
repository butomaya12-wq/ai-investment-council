...
from aic.council.rebuttal_eval_runtime import (
    ...
    build_rebuttal_eval_case_request,
    dry_run_manifest,
    execute_rebuttal_eval_case_once,
)
from aic.domain.canonical import canonical_sha256


ARTIFACT_VERSION = "B4_REBUTTAL_MODEL_EVAL_ARTIFACT_v0_1"
PAID_AUTHORIZATION_ARTIFACT_VERSION = "B4_REBUTTAL_MODEL_EVAL_PAID_AUTHORIZATION_v0_1"
PAID_CALL_RECEIPT_VERSION = "B4_REBUTTAL_MODEL_EVAL_PAID_CALL_RECEIPT_v0_1"
DEFAULT_REQUEST_PREFLIGHT = Path(".aic-runtime/b4_rebuttal_eval_request_preflight_v0_1.json")
DEFAULT_COST_PREFLIGHT = Path(".aic-runtime/b4_rebuttal_eval_cost_preflight_v0_1.json")
DEFAULT_DRY_OUTPUT = Path(".aic-runtime/b4_rebuttal_model_eval_dry_v0_1.json")
DEFAULT_PAID_OUTPUT = Path(".aic-runtime/b4_rebuttal_model_eval_v0_1.json")
DEFAULT_AUTHORIZATION_OUTPUT = Path(
    ".aic-runtime/b4_rebuttal_model_eval_paid_authorization_v0_1.json"
)
DEFAULT_RECEIPT_JOURNAL = Path(
    ".aic-runtime/b4_rebuttal_model_eval_paid_receipts_v0_1.jsonl"
)
EXPECTED_CASE_IDS = ("E4", "E8", "E13", "E16")
EXPECTED_MAX_OUTPUT_TOKENS = 6144


class RebuttalEvalAuthorizationError(ValueError):
    pass
...
def _args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Dry-run or execute the frozen B4 Rebuttal model evaluation. "
            "Paid dispatch requires exact request/cost owner approval."
        )
    )
...
def _read_object(path: Path, *, label: str) -> dict[str, Any]:
    try:
        ...
        raise RebuttalEvalAuthorizationError(f"unable to read {label}: {path}") from exc
    if not isinstance(value, dict):
        raise RebuttalEvalAuthorizationError(f"{label} root must be an object")
    return value


def _decimal(value: object, *, field_name: str) -> Decimal:
    if not isinstance(value, str):
        raise RebuttalEvalAuthorizationError(f"{field_name} must be decimal string")
    try:
        parsed = Decimal(value)
    except InvalidOperation as exc:
        raise RebuttalEvalAuthorizationError(f"{field_name} invalid decimal") from exc
    if not parsed.is_finite() or parsed < 0:
        raise RebuttalEvalAuthorizationError(f"{field_name} invalid")
    return parsed
...
def _verify_authorities(
    ...
    cost_hash = verify_rebuttal_eval_cost_preflight(cost_preflight)
    if cost_preflight.get("status") != REBUTTAL_EVAL_COST_PREFLIGHT_STATUS:
        raise RebuttalEvalAuthorizationError("Rebuttal eval cost preflight is not approval-ready")
    if cost_preflight.get("eval_request_preflight_artifact_hash") != request_hash:
        raise RebuttalEvalAuthorizationError("cost preflight does not bind request preflight")
    if cost_preflight.get("eval_request_manifest_hash") != request_preflight.get("request_manifest_hash"):
        raise RebuttalEvalAuthorizationError("cost/request manifest binding mismatch")
    if request_preflight.get("planned_paid_calls_max") != EXPECTED_REBUTTAL_EVAL_PAID_CALLS_MAX:
        raise RebuttalEvalAuthorizationError("Rebuttal eval call ceiling drift")
    if cost_preflight.get("planned_paid_calls_max") != EXPECTED_REBUTTAL_EVAL_PAID_CALLS_MAX:
        raise RebuttalEvalAuthorizationError("Rebuttal eval cost call ceiling drift")
    if request_preflight.get("max_output_tokens_per_call") != EXPECTED_MAX_OUTPUT_TOKENS:
        raise RebuttalEvalAuthorizationError("Rebuttal eval output-token bound drift")
    if cost_preflight.get("max_output_tokens_per_call") != EXPECTED_MAX_OUTPUT_TOKENS:
        raise RebuttalEvalAuthorizationError("Rebuttal eval cost output-token bound drift")
    if tuple(request_preflight.get("case_ids", ())) != EXPECTED_CASE_IDS:
        raise RebuttalEvalAuthorizationError("Rebuttal eval case surface drift")
    if tuple(request_preflight.get("candidate_keys", ())) != tuple(
        candidate.candidate_key for candidate in REBUTTAL_MODEL_LADDER
    ):
        raise RebuttalEvalAuthorizationError("Rebuttal model ladder drift")
    if request_preflight.get("initial_council_freeze_artifact_hash") != EXPECTED_INITIAL_FREEZE_HASH:
        raise RebuttalEvalAuthorizationError("Rebuttal eval Initial freeze binding drift")
    if cost_preflight.get("initial_council_freeze_artifact_hash") != EXPECTED_INITIAL_FREEZE_HASH:
        raise RebuttalEvalAuthorizationError("Rebuttal eval cost Initial freeze binding drift")
    if cost_preflight.get("pricing_hash") != EXPECTED_PRICING_HASH:
        raise RebuttalEvalAuthorizationError("Rebuttal eval pricing hash drift")
    for obj, label in ((request_preflight, "request"), (cost_preflight, "cost")):
        if obj.get("automatic_repair_calls_authorized") is not False:
            raise RebuttalEvalAuthorizationError(f"{label} unexpectedly authorizes automatic repair")
        for field in ("model_calls", "provider_reads", "broker_writes", "alpaca_orders"):
            if obj.get(field) != 0:
                raise RebuttalEvalAuthorizationError(f"{label} preflight {field} must be zero")
        if obj.get("live_money") != "PROHIBITED":
            raise RebuttalEvalAuthorizationError(f"{label} live-money invariant drift")
        for field in (
            "paid_eval_authorized",
            "production_rebuttal_authorized",
            "judge_authorized",
            "rerun_authorized",
        ):
            if field in obj and obj.get(field) is not False:
                raise RebuttalEvalAuthorizationError(f"{label} unexpectedly sets {field}")
    ceiling = _decimal(
        cost_preflight.get("total_rebuttal_eval_cost_upper_bound_usd"),
        field_name="total_rebuttal_eval_cost_upper_bound_usd",
    )
...
def _dry_run(
    ...
    request_hash, cost_hash, ceiling = _verify_authorities(request_preflight, cost_preflight)
    manifest = dry_run_manifest()
    if manifest.get("request_count") != EXPECTED_REBUTTAL_EVAL_PAID_CALLS_MAX:
        raise RebuttalEvalAuthorizationError("dry-run does not contain exactly 12 requests")
    expected = request_preflight.get("request_variants")
    if not isinstance(expected, list) or len(expected) != EXPECTED_REBUTTAL_EVAL_PAID_CALLS_MAX:
        raise RebuttalEvalAuthorizationError("request-preflight variants missing")
    observed = manifest.get("requests")
    if not isinstance(observed, list) or len(observed) != len(expected):
        raise RebuttalEvalAuthorizationError("dry-run request surface missing")
    for frozen, rebuilt in zip(expected, observed, strict=True):
        if not isinstance(frozen, Mapping) or not isinstance(rebuilt, Mapping):
            raise RebuttalEvalAuthorizationError("dry-run request record malformed")
        for field in (
            "candidate_key",
            "model",
            "reasoning_effort",
            ...
            if frozen.get(field) != rebuilt.get(field):
                raise RebuttalEvalAuthorizationError(
    ...
    if rebuilt_manifest_hash != request_preflight.get("request_manifest_hash"):
        raise RebuttalEvalAuthorizationError("dry-run request manifest hash drift")
    return {
        "manifest": manifest,
...
def validate_paid_execution_authorization(
    *,
    ...
    if approve_request_preflight_hash != request_hash:
        raise RebuttalEvalAuthorizationError("paid Rebuttal eval requires exact request-preflight approval")
    if approve_request_manifest_hash != request_preflight.get("request_manifest_hash"):
        raise RebuttalEvalAuthorizationError("paid Rebuttal eval requires exact request-manifest approval")
    if approve_cost_artifact_hash != cost_hash:
        raise RebuttalEvalAuthorizationError("paid Rebuttal eval requires exact cost-artifact approval")
    if _decimal(approve_max_usd, field_name="approve_max_usd") != ceiling:
        raise RebuttalEvalAuthorizationError("paid Rebuttal eval requires exact cost-ceiling approval")
    return ceiling


def _canonical_owner_approval_time(value: str | None) -> str:
    if not isinstance(value, str) or not value.strip():
        raise RebuttalEvalAuthorizationError("owner approval timestamp is required")
    text = value.strip()
    parse_value = text[:-1] + "+00:00" if text.endswith("Z") else text
    try:
        parsed = datetime.fromisoformat(parse_value)
    except ValueError as exc:
        raise RebuttalEvalAuthorizationError("owner approval timestamp must be RFC3339") from exc
    if parsed.tzinfo is None or parsed.utcoffset() != UTC.utcoffset(parsed):
        raise RebuttalEvalAuthorizationError("owner approval timestamp must be UTC")
    if parsed > datetime.now(UTC):
        raise RebuttalEvalAuthorizationError("owner approval timestamp cannot be future")
...
def validate_owner_approval_record(
    ...
    if not isinstance(owner_approval_id, str) or not owner_approval_id.strip():
        raise RebuttalEvalAuthorizationError("owner approval ID is required")
    approval_id = owner_approval_id.strip()
    if len(approval_id) > 160 or any(ch.isspace() for ch in approval_id):
        raise RebuttalEvalAuthorizationError("owner approval ID must be <=160 chars without whitespace")
...
def _git_execution_context(expected_head: str) -> dict[str, Any]:
    try:
        ...
        raise RebuttalEvalAuthorizationError("unable to prove local git execution context") from exc
    if head != expected_head:
        raise RebuttalEvalAuthorizationError("local HEAD differs from approved Rebuttal eval HEAD")
    if status.strip():
        raise RebuttalEvalAuthorizationError("paid Rebuttal eval requires clean git worktree")
...
def _run_id(started: str, request_hash: str, cost_hash: str) -> str:
    ...
    return f"AIC-B4-REBUTTAL-EVAL-{compact}-{suffix}"
...
def _authorization_artifact(
    *,
    ...
    artifact: dict[str, Any] = {
        ...
        "run_class": "B4_REBUTTAL_MODEL_EVAL_PAID_PRE_DISPATCH_AUTHORIZATION",
...
def _receipt(
    *,
    ...
    semantic_replay = "COMPLETE" if run.structured_output is not None else "INCOMPLETE"
    ...
    receipt: dict[str, Any] = {
        ...
        "semantic_replay_status": semantic_replay,
...
def _candidate_record(candidate: Any, runs: tuple[Any, ...]) -> tuple[dict[str, Any], StageModelEvalResult]:
    passed = all(run.passed for run in runs)
    critical_failures = sum(1 for run in runs if run.critical_safety and not run.passed)
    cost = sum((run.actual_cost_usd or Decimal("0") for run in runs), Decimal("0"))
...
def _blocked_artifact(
    *,
    ...
    artifact: dict[str, Any] = {
        ...
        "run_class": "B4_REBUTTAL_MODEL_EVAL",
        "status": status,
...
def main() -> int:
    args = _args()
    try:
        request_preflight = _read_object(args.request_preflight, label="request preflight")
        cost_preflight = _read_object(args.cost_preflight, label="cost preflight")
        dry = _dry_run(request_preflight, cost_preflight)

        if not args.execute_paid_eval:
            artifact: dict[str, Any] = {
                "artifact_version": ARTIFACT_VERSION,
                "run_class": "B4_REBUTTAL_MODEL_EVAL_DRY_RUN",
                "status": "READY_FOR_EXPLICIT_OWNER_B4_REBUTTAL_MODEL_EVAL_AUTHORIZATION",
                "eval_version": REBUTTAL_EVAL_VERSION,
                "runtime_version": REBUTTAL_EVAL_RUNTIME_VERSION,
                "model_policy_version": MODEL_POLICY_VERSION,
            ...
            artifact["artifact_hash"] = canonical_sha256(artifact)
            _write(args.dry_output, artifact)
        ...
        owner_id, owner_at = validate_owner_approval_record(
            args.owner_approval_id, args.owner_approval_at_utc
        )
        ...
        if len(frozen_variants) != EXPECTED_REBUTTAL_EVAL_PAID_CALLS_MAX:
            raise RebuttalEvalAuthorizationError("frozen request lookup count mismatch")
        ...
        for candidate in REBUTTAL_MODEL_LADDER:
            runs = []
            for case in cases:
                if dispatch_attempts >= EXPECTED_REBUTTAL_EVAL_PAID_CALLS_MAX:
                    raise RebuttalEvalAuthorizationError("paid Rebuttal eval dispatch ceiling exhausted")
                ...
                if frozen.get("request_hash") != request.request_hash:
                    raise RebuttalEvalAuthorizationError("paid request hash differs from preflight")
                if frozen.get("request_body_utf8_bytes") != request_bytes:
                    raise RebuttalEvalAuthorizationError("paid request bytes differ from preflight")
                if request.request_payload.get("max_output_tokens") != EXPECTED_MAX_OUTPUT_TOKENS:
                    raise RebuttalEvalAuthorizationError("paid request output cap drift")

                print(
                    f"[B4 REBUTTAL EVAL] {candidate.candidate_key} {case.case_id} "
                ...
                run = execute_rebuttal_eval_case_once(
                    case,
                ...
                if tracker.dispatch_attempts != 1:
                    raise RebuttalEvalAuthorizationError(
                        "each Rebuttal eval case must attempt exactly one provider dispatch"
                    )
                dispatch_attempts += 1
                ...
                if tracker.provider_responses != 1 or run.model_calls != 1:
                    artifact = _blocked_artifact(
                        ...
                        reason=run.findings[0] if run.findings else "provider response unavailable",
                        run_id=run_id,
                        code_commit_sha=git_context["code_commit_sha"],
                        request_preflight=request_preflight,
                        cost_preflight=cost_preflight,
                ...
                if run.cost_receipt_status != "COMPLETE" or run.actual_cost_usd is None:
                    artifact = _blocked_artifact(
                        ...
                        reason=run.findings[-1] if run.findings else "cost receipt incomplete",
                        run_id=run_id,
                        code_commit_sha=git_context["code_commit_sha"],
                        request_preflight=request_preflight,
                        cost_preflight=cost_preflight,
        ...
        if dispatch_attempts != EXPECTED_REBUTTAL_EVAL_PAID_CALLS_MAX:
            raise RebuttalEvalAuthorizationError("paid Rebuttal eval dispatch count is not 12")
        if completed_model_responses != EXPECTED_REBUTTAL_EVAL_PAID_CALLS_MAX:
            raise RebuttalEvalAuthorizationError("paid Rebuttal eval provider-response count is not 12")
        if len(receipt_hashes) != EXPECTED_REBUTTAL_EVAL_PAID_CALLS_MAX:
            raise RebuttalEvalAuthorizationError("paid Rebuttal eval receipt count is not 12")
        ...
        artifact: dict[str, Any] = {
            ...
            "semantic_replay_receipts_complete": sum(
                1
                for record in candidate_records
                for case_record in record["cases"]
                if case_record["structured_output_hash"] is not None
            ),
            "paid_call_receipt_hashes": receipt_hashes,
            "receipt_manifest_hash": canonical_sha256({"receipt_hashes": receipt_hashes}),
            ...
            json.dumps(
                {
                    ...
                    "semantic_replay_receipts_complete": artifact[
                        "semantic_replay_receipts_complete"
                    ],
        ...
        print(
            f"B4 Rebuttal model eval failed closed: {type(exc).__name__}: {exc}",
            file=sys.stderr,
        )
        return 2
