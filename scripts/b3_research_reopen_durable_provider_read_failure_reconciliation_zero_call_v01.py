...
DEFAULT_OUTPUT = Path(".aic-runtime/b3_research_reopen_durable_provider_read_failure_reconciliation_zero_call_v0_1.json")
...
def _read_json(path: Path, *, label: str) -> dict:
    try:
        ...
        raise DurableProviderReadFailureReconciliationError(f"unable to read {label}") from exc
    if not isinstance(payload, dict):
        raise DurableProviderReadFailureReconciliationError(f"{label} root must be object")
    return payload
...
def _read_jsonl(path: Path, *, label: str) -> list[dict]:
    ...
    for index, raw in enumerate(lines, start=1):
        ...
        try:
            ...
            raise DurableProviderReadFailureReconciliationError(
...
def main() -> int:
    ...
    try:
        ...
        if _git("branch", "--show-current") != "hackathon/alpaca-2026":
            raise DurableProviderReadFailureReconciliationError(
                "reconciliation requires hackathon/alpaca-2026 branch"
            )
        if _git("status", "--porcelain"):
            raise DurableProviderReadFailureReconciliationError(
                "reconciliation requires clean worktree"
            )
        ...
        print(f"DurableProviderReadFailureReconciliationError: {exc}")
        print("MODEL_CALLS=0")
        print("MODEL_SYNTHESIS_CALLS=0")
        print("PROVIDER_READS=0")
        print("BROKER_WRITES=0")
