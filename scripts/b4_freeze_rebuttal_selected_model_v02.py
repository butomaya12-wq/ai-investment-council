...
DEFAULT_OUTPUT = Path(".aic-runtime/b4_rebuttal_selected_model_authority_v0_2.json")
...
def main() -> int:
    ...
    for field, expected in exact.items():
        if eval_artifact.get(field) != expected:
            raise ValueError(
                f"paid Rebuttal eval exact evidence drift: {field}"
            )

    selection = eval_artifact.get("selection")
    if (
        not isinstance(selection, dict)
        or selection.get("selected_candidate") != EXPECTED_SELECTED
    ):
        raise ValueError("paid Rebuttal eval selected candidate drift")

    if any("result_hash" in receipt for receipt in receipts):
        raise ValueError(
            "historical production receipt contract unexpectedly changed; "
            "v0.2 is intended to replay result_hash from durable receipt fields"
        )
    ...
    selection_hash = verify_rebuttal_selected_model_authority_v02(authority)

    if authority.get("artifact_version") != REBUTTAL_SELECTED_MODEL_AUTHORITY_VERSION_V02:
        raise ValueError("zero-call selection authority version drift")
    if authority.get("replay_contract_version") != REPLAY_CONTRACT_VERSION:
        raise ValueError("zero-call replay contract version drift")
    if authority.get("replayed_result_hash_count") != 12:
        raise ValueError("expected twelve reconstructed result hashes")
    ...
    print(
        json.dumps(
            {
                "status": "PASS_ZERO_CALL_REBUTTAL_SELECTED_MODEL_FREEZE_V02",
                "artifact_version": authority["artifact_version"],
                "replay_contract_version": authority["replay_contract_version"],
                "source_paid_eval_head": authority["source_git_commit"],
                "paid_run_id": authority["paid_run_id"],
                "model_eval_artifact_hash": authority[
                    "model_eval_artifact_hash"
                ],
                "paid_authorization_artifact_hash": authority[
                    "paid_authorization_artifact_hash"
                ],
                "receipt_manifest_hash": authority["receipt_manifest_hash"],
                "semantic_replay_receipts_complete": authority[
                    "semantic_replay_receipts_complete"
                ],
                "semantic_replay_passed_cases": authority[
                    "semantic_replay_passed_cases"
                ],
                "replayed_result_hash_count": authority[
                    "replayed_result_hash_count"
                ],
                "selected_candidate": authority["selected_candidate"],
                "selection_reason_code": authority[
                    "selection_reason_code"
                ],
                "selected_eval_metrics": authority[
                    "selected_eval_metrics"
                ],
                "full_ladder_pass_summary": authority[
                    "full_ladder_pass_summary"
                ],
                "actual_paid_eval_cost_usd": authority[
                    "actual_paid_eval_cost_usd"
                ],
