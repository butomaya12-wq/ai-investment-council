...
from aic.council.initial_runtime_diagnosis_v04 import (
    diagnose_blocked_initial_runtime_v04,
)
...
DEFAULT_OUTPUT = Path(".aic-runtime/b4_initial_runtime_block_diagnosis_v0_4.json")
...
def main() -> int:
    ...
    if runtime_preflight.get("initial_schema_repair_version") != INITIAL_SCHEMA_REPAIR_VERSION:
        raise ValueError("runtime preflight does not bind v0.4 schema repair")
    if runtime_preflight.get("initial_schema_version") != INITIAL_SCHEMA_VERSION:
        raise ValueError("runtime preflight does not bind v0.4 repaired schema")
    if (
        runtime_preflight.get("initial_promotion_semantics_contract_version")
        != PROMOTION_SEMANTICS_CONTRACT_VERSION
    ):
        raise ValueError("runtime preflight does not bind v0.4 promotion contract")

    authority = load_initial_selected_model_authority()
    model_inputs = build_initial_model_inputs(freeze, reconciliation, handoff)

    artifact = diagnose_blocked_initial_runtime_v04(
    ...
    if artifact["artifact_hash"] != expected_hash:
        raise ValueError("v0.4 diagnosis artifact hash mismatch")
    ...
    public = {
        ...
        "successful_replay_count": artifact["successful_replay_count"],
        "blocked_replay_count": artifact["blocked_replay_count"],
        "initial_stage_forbidden_claim_type_count": artifact[
            "initial_stage_forbidden_claim_type_count"
        ],
        "promotion_rule_failure_count": artifact["promotion_rule_failure_count"],
        "non_stage_promotion_rule_failure_count": artifact[
