...
def verify_resume_owner_approval(approval:Mapping[str,Any],*,code_head:str,readiness_hash:str,cost:Mapping[str,Any])->str:
    observed=_hash(approval); expected=build_resume_owner_approval(code_head=code_head,readiness_hash=readiness,cost=cost,owner_approval_id=str(approval.get("owner_approval_id","")),owner_approval_at_utc=str(approval.get("owner_approval_at_utc",""))); _need(dict(approval)==expected,"resume owner approval drift"); return observed

def execute_paid_recovery_resume(*,execute_paid_recovery_resume:bool,branch:str,code_head:str,worktree_clean:bool,cost:Mapping[str,Any],readiness:Mapping[str,Any],reconciliation:Mapping[str,Any],approval:Mapping[str,Any]|None,ledger_path:Path,raw_dir:Path,result_path:Path,transport_factory:Callable[[],Callable[[Mapping[str,Any]],Mapping[str,Any]]],now:Callable[[],datetime]=lambda:datetime.now(UTC))->dict[str,Any]:
    ...
    _need(readiness.get("code_commit_sha")==code_head and readiness.get("new_paid_calls_planned")==8 and readiness.get("resume_request_1_included") is False,"final resume readiness drift")
    _need(reconciliation.get("status")==RECON_STATUS and reconciliation.get("request_1_resend_allowed") is False,"reconciliation drift"); approval_hash=verify_resume_owner_approval(approval,code_head=code_head,readiness_hash=str(readiness.get("artifact_hash")),cost=cost)
