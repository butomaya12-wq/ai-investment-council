...
def test_planner_structured_output_constrains_sec_sections_to_runtime_capability() -> None:
    ...
    assert enum_values == list(RUNTIME_RETRIEVABLE_SEC_SECTION_VALUES)
    assert enum_values == ["Business", "Risk Factors", "MD&A"]
    assert "Material 8-K" not in enum_values
